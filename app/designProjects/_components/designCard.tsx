...
export default function DesignCard({ imgSrc, title, description, tag, url }: DesignCardProps) {
  return (
    <article className="group flex w-full flex-col gap-5 rounded-inner bg-white/18 p-4 shadow-lg shadow-primary/8 transition-transform duration-500 hover:-translate-y-0.5 md:p-6">
      <div className="relative aspect-5/3 w-full overflow-hidden rounded-nested bg-black/70">
      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-wrap items-start justify-between gap-x-5 gap-y-2">
          <h2 className="subtitle py-0!">{title}</h2>
          <p className="rounded-full bg-white/32 px-3 py-1.5 text-xs font-medium tracking-wide text-secondary">{tag.join(" / ")}</p>
        </div>
        <p className="text-sm leading-relaxed text-primary">{description}</p>
        <a href={url} target="_blank" rel="noreferrer noopener" className="inline-flex w-full items-center justify-center rounded-inner bg-white/30 px-4 py-3 text-sm font-semibold tracking-wide text-secondary transition-colors hover:bg-white/50 hover:text-primary focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-secondary">
          查看專案
        </a>
      </div>
    </article>
